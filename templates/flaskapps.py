        ...
        for (uid, emails, usrpwd, giftcycleid, comp) in myresult:
            if (emails == mail):
                if (comp == 'I'):
                    ...
                    server.quit()
                    admin = uid
                    return render_template('newlogin.html', otps=otp, admin=0, otp=0, mail=mail, smt=1)
                else:
                    auth = usrpwd
                    if (uid == 0):
                        admin = uid
    ...
    if session['loggedin'] == True:
        try:
            if (admin):
                ...
                try:
                    return render_template('admin.html', mail=mail, resi=resi, gcycle=gfcycle, thambu=bus, temp=temp_admin, toos=totalorders, counts=counts, mys=mys, fn=realname)
                except:
...
def logout():
    ...
    session.pop('mail', None)
    session.clear()
    # flash('U click on back button so u logged out')
    # mrd=1
    return render_template('newlogin.html')
...
def dashboardbutton(id_data):
    ...
    return render_template('admin.html', mail=mail, resi=resi, gcycle=gfcycle, thambu=bus, temp=temp_admin, toos=totalorders, counts=counts, mys=mys, fn=realname,ide=id_data)
...
def forgot_pwd():
    ...
    if (forgot):
        ...
        return render_template('newlogin.html', otps=otp, admin=0, otp=0, mail=email, smt=1, ld=1)
...
def veri():
    ...
    if (otp == ab):
        ...
        return render_template('newlogin.html', otps=otp, admin=0, otp=0, mail=mail, smt=1, ld=2)
    else:
...
def ship_inti():
    ...
    try:
        ...
        try:
            ...
            return render_template('admin.html', mail=mail, resi=resi, gcycle=gfcycle, thambu=bus, temp=temp_admin, toos=totalorders, counts=counts, mys=mys, cs=csvname, held=held,usrq=usr)
        except:
            session['_flashes'].clear()
            session.pop('loggedin', None)
            session.pop('mail', None)
...
def dele(id_data):
    ...
    try:
        ...
        try:
            ...
            return render_template('admin.html', mail=mail, resi=resi, gcycle=gfcycle, thambu=bus, temp=temp_admin, toos=totalorders, counts=counts, mys=mys)
        except:
            session['_flashes'].clear()
            session.pop('loggedin', None)
            session.pop('mail', None)
    ...
    if uploaded_file.filename != '':
        ...
        for i, row in csvData.iterrows():
            try:
                ...
                return render_template('admin.html', mail=mail, resi=resi, gcycle=gfcycle, temp=temp_admin, cs=csvname, thambu=bus, toos=totalorders, counts=counts, mys=mys,usrq='3')
    ...
    <html>
      ...
      <body style="background-color:White;">
       ...
       <h1>
         ...
         <a href="https://www.w3schools.com" style="height:200px; background-color:blue; color:white; text-decoration:none; padding:10px; margin-bottom:10px;"> Activate your account</a>
         <br>
         <br>
